    ...
    if (arguments[0] == 'help') {
      ...
      switch(arguments[0]) {
        case 'assets':
          if (arguments.length > 1) {
            ...
            files['asset.css'] = {file: path.join(target.stylesheets, name.name + '.css'), name: name};
            files['asset.js'] = {file: path.join(target.javascripts, name.name + '.js'), name: name};
          } else {
      ...
      if (Object.keys(files).length != 0) {
        Object.keys(files).forEach(function (name) {
          app.log.info('Writing file ' + files[name].file.slice(root.length+1).grey);
        });

        module.exports.writeFiles(generator, files, callback);
      }
    } else {
      displayHelp = true;
    }
...
module.exports.writeFiles = function (source, files, next) {
  ...
  var copyFile = function (file, nextFile) {
    fs.readFile(path.join(source, file), function (err, data) {
      ...
      Object.keys(inflections).forEach(function (key) {
        data = data.gsub(inflections[key], files[file].name[key]);
      });

      fs.writeFile(files[file].file, data, nextFile);
    });
  }

  common.async.forEachSeries(Object.keys(files), copyFile, next);
}
