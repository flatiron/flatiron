  ...
  if (arguments.length > 1) {
    if (arguments[0] == 'help') {
      ...
      switch(arguments[0]) {
        case 'assets':
          if (arguments.length > 1) {
            var name = inflect.underscore(inflect.pluralize((arguments[1])));
            files.push({
              src: 'asset.css',
              conf: {name: name},
        ...
        case 'resource':
          if (arguments.length > 1) {
            var name = inflect.underscore(inflect.singularize(arguments[1]));
            var camel = inflect.camelize(inflect.singularize(arguments[1]));
            var prop = '';
