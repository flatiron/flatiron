...
exports.attach = function (options) {
  ...
  app.router = new director.http.Router(app.http.routes).configure({
    async: true
  });

  if (app.http.attach) {
    app.router.attach(function () {
      for (var i in app.http.attach) {
        this[i] = app.http.attach[i];
      }
    });
  }
  ...
  app.listen = function (port, host, callback) {
    ...
    app.server = union.createServer({
      ...
      before: app.http.before.concat(function (req, res) {
        if (!app.router.dispatch(req, res, app.http.onError || exports._errorHandler)) {
...
exports._errorHandler = function (err) {
  if (err) {
    this.res.text(err.status || 500, err.stack || err.body.error);
  }
  this.res.text(500, 'Unknown error');
};
