...
exports.attach = function (options) {
  ...
  self.start = function (callback) {
    self.cli.router.dispatch('on', process.argv.slice(2).join(' '), callback);
  };
  
  if(options.dir) {
    self.cli.router.notfound = function findModule(callback) {
      var parts = this.cmd.split(new RegExp(self.cli.router.delimiter)),
        needle,
        usageTarget = options.usage;
      ...
      while (needle) {
        ...
        if (typeof needle === 'function' && needle.length === parts.length) {
