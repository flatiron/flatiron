...
exports._routesBuilder = function (scope) {
  ...
  this.root = function (action) {
    ...
    if (exports._checkAction(action)) {
      this._scope['/'] = { on: app.presenters[action[0]][action[1]] };
    }
  };

  this.match = function (url, action) {
    action = action.split('/');
    if (exports._checkAction(action)) {
      this._scope['/' + url] = { on: app.presenters[action[0]][action[1]] };
    }
  };
  ...
  this.scope = function (name, func) {
    scope[name] = {};
    func.call(new exports._routesBuilder(scope[name]));
  };

  this.get = function (action) {
    action = action.split('/');
    if (exports._checkAction(action)) {
      this._scope['/' + action.join('/')] = { get: app.presenters[action[0]][action[1]] };
    }
  };
  ...
  this.post = function (action) {
    action = action.split('/');
    if (exports._checkAction(action)) {
      this._scope['/' + action.join('/')] = { post: app.presenters[action[0]][action[1]] };
    }
  };
