...
exports.attach = function (options) {
  ...
  app.start = function (callback) {
    app.router.dispatch('on', process.argv.slice(2).join(' '), app.log, callback);
  };
...
exports.commands = function (options) {
  ...
  app.router.notfound = function processCommands(callback) {
    if (app.cli.dir) {
      var parts = app.argv._.slice(),
          part = parts.shift(),
          command,
          usage;
    
      try {
        command = require(path.join(options.dir, part));
      }
      catch (err) {
