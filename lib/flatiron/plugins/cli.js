...
var sugarskull = require('sugarskull'),
  LazyScaffold = require('../util/lazyscaffold');
...
exports.init = function (next) {
  ...
  if (self.options[exports.name].dir) {
    exports.scaffold = new LazyScaffold(options.dir);
    scaffold.list(function (err, files) {
      if (err) {
        return next(err);
      }
      files.forEach(function (file) {
        file = file.replace(/[.][\s\S]*$/, '');
        self.cmd(file, function(path, handler) {
          ;
        });
      });
      return next();
